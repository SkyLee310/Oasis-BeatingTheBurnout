import {
  Award, CalendarCheck, CheckCheck, Clock, HeartHandshake, History, Lock,
  Scale, Shield, X,
} from 'lucide-react'
...
export default function TrackRecordSheet({ onClose }: { onClose: () => void }) {
  ...
  const titleId = 'track-record-title'
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex flex-col gap-1 min-w-0">
            <h2 id={titleId} className="t-sub text-ink">Your track record</h2>
            <span className="t-micro" style={{ color: 'var(--ink-muted)' }}>
              Your own history — not a rating, and not anybody's opinion of you.
            </span>
          </div>
          <button
        ...
        {empty ? (
          <div className="tile tile-cream" style={{ padding: 20, gap: 10 }}>
            ...
            <p className="t-body max-w-[46ch]" style={{ color: 'var(--ink-2)' }}>
              Your record fills in as group projects end — what you carried, what you
              closed, and whether it went in on time. Nothing here is written by
              anybody but the projects themselves.
            </p>
          </div>
        ) : (
          <>
            ...
            >
              ...
              <Stat value={`${record.weightShare}%`} label="of team weight" divide />
            </div>
            ...
            <section className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2">
                {record.projects.map(p => <ProjectCard key={p.id} project={p} />)}
              </div>
            </section>
                ...
                <div className="flex flex-col gap-2">
                  {record.achievements.map(a => (
                    <div key={a.id} className="flex items-start gap-3">
                      ...
                      >
                        {ACHIEVEMENT_ICON[a.id] ?? <Award size={16} strokeWidth={SW} />}
                      </span>
                      <div className="flex flex-col gap-1 min-w-0">
        ...
        >
          <div className="flex items-center gap-2">
            {shared ? <Shield size={15} strokeWidth={SW} /> : <Lock size={15} strokeWidth={SW} />}
          ...
          <button
            className={`btn ${shared ? 'btn-secondary' : 'btn-primary'} focus-ring w-fit`}
          ...
          <p className="t-micro" style={{ color: 'var(--ink-muted)', lineHeight: 1.5 }}>
            No peer ratings. No leaderboard. Nothing here is visible unless you share it.
          </p>
        </div>
      </div>
    </div>
...
function Stat({ value, label, divide }: { value: string; label: string; divide?: boolean }) {
  return (
    ...
    >
      <span className="t-stat text-ink" style={{ fontSize: 30, lineHeight: 1 }}>{value}</span>
      <span className="t-micro text-center" style={{ color: 'var(--ink-2)' }}>{label}</span>
    </div>
  )
}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-0.5 min-w-0">
          ...
          <span className="t-micro" style={{ color: 'var(--ink-muted)' }}>{project.title}</span>
        </div>
