      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto flex flex-col gap-3 px-5 py-5" style={{ background: 'var(--canvas)' }}>
        {messages.map((m, i) => (
          <div key={i} className={`flex items-end gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {m.role === 'ai' && <OasisBlob zone="amber" size={34} float={false} fillColor="var(--color-amber-100)" strokeColor="var(--color-black)" mouthFill="var(--color-amber-100)" mouthStroke="var(--color-black)" />}
            <div
              className="t-label"
              style={{
                maxWidth: '80%', padding: '11px 15px',
                background: m.role === 'user' ? 'var(--ink)' : 'rgba(254, 230, 133, 0.65)',
                color: m.role === 'user' ? 'var(--on-ink)' : 'var(--surface-darkest)',
      ...
      <div className="flex flex-col gap-2.5 px-4 py-4 rule-t" style={{ background: 'var(--surface)' }}>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            style={{ background: 'rgba(20, 71, 230, 0.55)', color: 'var(--color-black)' }}
          >
            <Send size={16} strokeWidth={SW} style={{ color: 'var(--color-black)' }} />
          </button>
        </div>
