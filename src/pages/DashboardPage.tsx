...
export default function DashboardPage({ onGoLoad, onGoRecovery, onGoBand, onGoPhone, onGoHow }: {
  ...
  return (
    <div className="flex flex-col gap-10 sm:gap-14 max-w-[1140px] mx-auto pb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] items-end gap-8 lg:gap-12">
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="flex items-center justify-center gap-2 sm:gap-6">
              <OasisBlob
                zone={zone} size={124} shadow
                fillColor="var(--zone-amber-bg)"
                strokeColor="rgb(0, 0, 0)"
                eyeHighlightFill="var(--zone-amber-bg)"
                mouthFill="var(--zone-amber-bg)"
                mouthStroke="rgb(0, 0, 0)"
              />
              <CircularGauge
      ...
      <section className="panel-ink p-6 sm:p-9 flex flex-col gap-4" style={{ backgroundColor: 'rgba(20, 20, 15, 0.1)', borderColor: 'rgba(20, 20, 15, 0.35)' }}>
        <div className="flex items-center justify-between gap-3">
          <span className="t-eyebrow inline-flex items-center gap-2" style={{ color: 'var(--text-secondary)' }}>
        ...
        <p className="t-display max-w-[26ch]" style={{ color: 'var(--brand-primary)' }}>
          &ldquo;{currentQuote.quote}&rdquo;
        </p>
        <span className="t-label" style={{ color: 'var(--ink-2)' }}>— {currentQuote.author}</span>
      </section>
      ...
      <section className={`grid grid-cols-1 gap-6 lg:gap-8 ${pending ? 'lg:grid-cols-[1.6fr_1fr]' : ''}`}>
        ...
        {pending && (
          <aside className="tile tile-butter card-pop self-start" style={{ padding: 22, gap: 14, background: 'var(--color-amber-100)' }}>
            <div className="flex items-start gap-3">
              <span
                className="flex items-center justify-center shrink-0"
                style={{
                  width: 40, height: 40, borderRadius: 999,
                  background: 'rgba(207, 40, 40, 0.6)', border: '2px solid var(--ink)',
                }}
              >
